function [z,a,it,ord,s,fct] = backcor1(n,y,ord,s,fct)
...
order = 1;
x1=linspace(0,15,length(y));
yy2 = smooth(x1,y,0.018,'rloess');
TF=islocalmin(yy2);
TF2=islocalmax(yy2);
max=yy2(TF2);
min=yy2(TF);
...
coefficients = polyfit(n, ztmp, 1);

% Slope text

slopelbl = uicontrol('Style','text','String','Slope :','HorizontalAlignment','left','BackgroundColor',bgclr);
slopevalue = uicontrol('Style','text','String',num2str(coefficients(1)),'BackgroundColor',bgclr);
...
% Callback functions
    ...
    function CostFunctionFcn(~,~)
        ...
        coefficients = polyfit(n, ztmp, 1);
        set(slopevalue,'String',num2str(coefficients(1)))
    end
    ...
    function [ztmp,atmp,ittmp,order,threshold,costfunction] = compute(n,y,order,threshold,costfunction)
        ...
        [ztmp,atmp,ittmp,order,threshold,costfunction] = backcor1(n,y,order,threshold,costfunction);
        [~,i] = sort(n);
    ...
    function WinResizeFcn(~,~)
        ...
        set(slopelbl,'Position',[w-240,h-180,220,20]);
        set(slopevalue,'Position',[w-240,h-200,220,20]);
    end
